...
class test_parameter(object):
    ...
    def setup(self):
        self.lit_int = pyf.Parameter('int_param', dtype=pyf.default_integer, expr="10+20")
        self.var_param = pyf.Parameter("var_param", dtype=pyf.default_integer, expr="int_param + 30")
        self.call_func = pyf.Parameter("call_func", dtype=pyf.default_integer, expr="selected_int_kind(10)")
...
class test_proc_params(object):
    ...
    def setup(self):
        ...
        func = pyf.Function(name="func", args=[self.real_arg, self.int_arg],
                                           params=[self.lit_int, self.sik, self.srk],
    ...
    def test_arg_man(self):
        ...
        func_am = pyf.ArgManager(args=self.args, return_arg=self.ret_arg, params=self.params)
        eq_(sub_am.arg_declarations(),
                ['integer(kind=fwrap_default_integer), parameter :: lit_int',
                 'integer(kind=fwrap_default_integer), parameter :: sik_10',
                 'integer(kind=fwrap_default_integer), parameter :: srk_10_20',
                 'real(kind=fwrap_srk_10_20), intent(inout) :: real_arg',
                 'integer(kind=fwrap_sik_10), dimension(lit_int) :: int_arg'])
